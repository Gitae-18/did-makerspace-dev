...
export default function ({ query }) {
    ...
    const getItemList = useCallback(async (pageNumber, searchWord) => {
        ...
        const limitCount = 20;
        let requri = PreUri + '/equipment/category?page=' + pageNumber + '&limit=' + limitCount;
   ...
   /*      const currPage = page - 1;
    ...
    const onSelectItem = useCallback((e, i) => {
        ...
        history((location.pathname+location.search+location.hash) + '?edit=3');
    ...
    const ItemRow = useCallback((props) => {
        return (<>
            <tr>
                ...
                <td className="eng" onClick={props.onClick}><span className="more">{props.name}</span></td>
                <td className="eng">{props.number}</td>
                <td className="eng">{props.spec}</td>
                <td>{props.purpose}</td>
        ...
        for (let i = 0; i <= items.items.length && i < items.limit; i++) {
            ...
            ItemRows.push(
                <ItemRow index={rowNumber}
                    ...
                    number={item.model_number ? item.model_number : '-'}
                    spec={item.model_specification ? item.model_specification : '-'}
    ...
    return (
        <div id="wrap" className="wrap management7">
            <div className="content_wrap">
                <SideNavi location={location} history={history} />
                <div className="content">
                    <div className="table">
                        <div className="search_box">
                            <input type="text" value={search} name="search" placeholder="품목명을 입력하세요" onChange={(e) => setSearch(e.target.value)}
