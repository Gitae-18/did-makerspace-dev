		...
		return(
		  <ol>
			  ...
			  <li className={sideNaviPos===9?"on":"off"}>
				<p onClick={(e)=>{onClick(e,9);}}>교육/행사관리</p>
			  </li>
			  </ol>
		)
	   }
...
export const SubBread2 = (props) => {
    ...
    return (
		...
		{props.subtitle&&props.subtitle.length>0?
         <MdHome className="homeicon" style={props.subtitle.length===8?{'left':'75px'}:props.subtitle.length===6?{'left':'80px'}:props.subtitle.length===5?{'left':'95px'}:props.subtitle.length===4?{'left':'80px'}:{'left':'80px'}}/>:
         <MdHome className="homeicon" style={props.title.length<4?{'left':'200px'}:props.title.length<5?{'left':'185px'}:props.title.length<6?{'left':'175px'}:props.title.lenth===6?{left:"170px"}:
         props.title.length<7?{'left':'170px'}: props.title.length<8?{'left':'160px'}:{'left':'150px'}}/>}
         
          {props.subtitle&&props.subtitle.length>0?<h2 style={props.subtitle.length === 3?{'left':'130px'}:props.subtitle.length === 4?{'left':'100px'}:props.subtitle.length === 5? {'left':'120px'}:props.subtitle.length === 6?{'left':'100px'}:{'left':'95px'}}>{props.title}</h2>:<h2 style={props.title.includes("오시는")?{'left':'203px'}:props.title.length===3?{'left':'225px'}:props.title.length===4?{'left':'205px'}:props.title.length===5?{'left':'195px'}:
          ...
          :props.subtitle.length===6?{'left':'155px'}:props.subtitle.length===8?{'left':'140px'}:{'left':'150px'}}/>:null}
        </div>
