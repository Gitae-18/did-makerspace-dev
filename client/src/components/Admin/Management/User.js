...
import { USER_ITEM } from "../../../store/management";
...
export default function ({  query }) {
    ...
    const getItemList = useCallback(async (pageNumber, searchWord) => {
        ...
        const limitCount = 20;
        let requri = PreUri + '/user?page=' + pageNumber + '&limit=' + limitCount;
    ...
    const onSelectItem = useCallback((e, i) => {
        e.preventDefault();
        dispatch({ type: USER_ITEM, target: items.items[i] });
        history((location.pathname+location.search+location.hash) + '?edit=2');
    ...
    const ItemRow = useCallback((props) => {
        return (<>
            <tr>
                ...
                <td className="eng">{props.email}</td>
                <td>{props.coName}</td>
                <td>{props.authLevel}</td>
                {/* <td>정상</td> */}
        ...
        for (let i = 0; i <= items.items.length && i < items.limit; i++) {
            ...
            ItemRows.push(
                <ItemRow index={rowNumber}
                    ...
                    authLevel={item.authority_level === 100 ? '시스템'
                        : item.authority_level === 90 ? '최고관리자'
                            : item.authority_level === 70 ? '운영자'
                                : item.authority_level === 50 ? '스케줄 관리자'
                                    : item.authority_level === 10 ? '파트 담당자'
                                        : '일반 사용자'}
    ...
    return (
        <div id="wrap" className="wrap management4">
            <div className="content_wrap">
                <SideNavi location={location} history={history} />
                <div className="content">
                    <div className="table">
                        <div className="search_box">
                            <input type="text" value={search} placeholder="회원명을 입력하세요" name="search" onChange={(e) => setSearch(e.target.value)}
                        ...
                        <button className="btn_apply" onClick={() => { history('/management?reg=2') }}>신규 등록</button>
